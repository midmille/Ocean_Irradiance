...
def Backscatter_Ratio_3(phy_type):
    ...
    it is probably necessary to multipply these by Carbon to Chla ratio but not sure exactly why. 

    Shouldn't they be dimensionless??

    This is a problem for another time. I am going to continue to use the two other methods which give ratios on the
    same order. bbr methods 1 and 2 give ratios around 0.001 ect, so much bigger than these. not too sure why.
    
    Also in an email on Mar 7, 2022
    """
    if species == 'HLPro': 
        return 0.000014685
    if species == 'LLPro': 
    ...
    if species == 'Diat':
        return 0.000000240
    if species == 'Generic': 
        return (0.000014685 + 0.000014685 + 0.000002516 +0.000000240 + 0.000013127 + 0.000001735 + 0.000051850)/7
    if species == 'Syn':
        return 0.000013127
    if species == 'Lgeuk':
        return 0.000001735
