...
def Plot_Single_Species_Correlation_Stats(year_min, cal_cast_dat, cal_bot_dat, cci_url, save_dir, save_head, PI, N, wavelengths, species, cci_wavelengths): 
    ...
    calchla_RMS = np.zeros(Nphy)
    calchla_rel_mean_bias = np.zeros(Nphy)

    ccichla_RMS = np.zeros(Nphy)
    ccichla_rel_mean_bias = np.zeros(Nphy)
    ...
    for k, phy_type in enumerate(species):
        ...
        ## calchla
        RMS, mean_bias, rel_mean_bias, mean_ratio, slope, intercept, N = Correlation_Stats(cal_chla, irr_chla)
        calchla_RMS[k] = RMS * 100
        calchla_rel_mean_bias[k] = rel_mean_bias*100

        ## ccichla
        RMS, mean_bias, rel_mean_bias, mean_ratio, slope, intercept, N = Correlation_Stats(cci_chla, irr_chla)
        ccichla_RMS[k] = RMS * 100
        ccichla_rel_mean_bias[k] = rel_mean_bias*100
    ...
    width = 0.65
    pos_phy = np.array([k for k in range(Nphy)])
    pos_phy1 = pos_phy - width/4
    pos_phy2 = pos_phy + width/4
    ax0.bar(pos_phy1, calchla_rel_mean_bias, color ='blue', alpha = 0.9, align='center', label=f'Relative Mean Bias', width =width/2)
    ax0.bar(pos_phy2, calchla_RMS, color ='grey', alpha = 0.9, align='center', label=f'RMSRE', width =width/2)

    ax1.bar(pos_phy1, ccichla_rel_mean_bias, color ='blue', alpha = 0.9, align='center', label=f'Relative Mean Bias', width =width/2)
    ax1.bar(pos_phy2, ccichla_RMS, color ='grey', alpha = 0.9, align='center', label=f'RMSRE', width =width/2)

    ax0.set_title("CalCOFI and Irradiance Model Chlorophyll-a Correlation")
    ax0.set_xticks(pos_phy)
    ax0.set_xticklabels(species, rotation=75)
    ax0.set_ylabel(r'Percent')
    ax0.grid(axis='y')

    ax1.set_title("CCI and Irradiance Model Chlorophyll-a Correlation")
    ax1.set_ylabel(r'Percent')
    ax1.set_xticks(pos_phy)
    ax1.set_xticklabels(species, rotation=75)
    ax1.grid(axis='y')
    ...
    ## [See if there is a correlations between rrs RMS and absorption/scattering.]
    ya = np.zeros(Nphy*Nlam)
    yb = np.zeros(Nphy*Nlam)
    x = np.zeros(Nphy*Nlam)

    i=0
    for k, phy_type in enumerate(species): 
        for j, lam in enumerate (cci_wavelengths): 
            ya[i] = abscat(lam, phy_type)[0]
            yb[i] = abscat(lam, phy_type)[1]
            x[i] = rrs_RMS[k,j]
            i+=1
    ...
    ax = PC.Plot_Comparison(ax, 
                            ...
                            '',## lable 
                            r'$R_{rs}$ RMSRE', 
                            r'Absorption $m^2 \mathrm{mgChla}^1$', 
                            color='blue', 
                            plot_slope=True, 
                            slope_color='blue', 
                            xlim = 0.01, 
    ...
    ax = PC.Plot_Comparison(ax, 
                            ...
                            '',## lable 
                            r'$R_{rs}$ RMSRE', 
                            r'Scattering $m^2 \mathrm{mgChla}^1$', 
                            color='blue', 
                            plot_slope=True, 
                            slope_color='blue', 
                            xlim = 0.01, 
