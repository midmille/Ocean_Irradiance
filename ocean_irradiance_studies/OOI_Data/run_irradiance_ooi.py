...
#                CDOM = OI.CDOM_chla(z_a, chla_intrp, lam)
                CDOM = OI.CDOM_refa(z_a, cdom_refa, cdom_reflam, lam, fraca=0.48)
...
#                             abscat(lam, 'detritus')[2])
                ...
                z_irr, a_irr, b_irr, b_b_irr, b_f_irr = OI.Calc_Abscat_Grid(PI, 
                                                                   z_a[0],
                                                                   abscat(lam, 'water'), 
                                                                   N, 
                                                                   PI.Ed0, 
...
def Plot_Irraddiance_SPKIR(prof_index, wavelengths, optaa_prof, spkir_prof, spkir_wavelengths, irr_field_ab, site, method): 
...
#        ax.plot(irr_arr[:, prof_index, 0]+irr_arr[:, prof_index, 1], irr_arr[:, prof_index, 3], ':', label=f'Irr {lam}', color=colors[k])
        ...
        ## make twin axis for abs and scat, share y. 
        lna = axa.plot(a, z_a, '-', color=colors[k], label=r'$a_{\mathrm{OOI}}$' + f'{lam}')
        lnb = axb.plot(b, z_a, ':', color=colors[k], label=r'$a_{\mathrm{OOI}}$' + f'{lam}')


        ax1.plot(irr_arr_ab[:, prof_index, 0]+irr_arr_ab[:, prof_index, 1], irr_arr_ab[:, prof_index, 3], '--', label=f'Model Using OOI ab {lam}', color=colors[k], linewidth=1.5)
#        ax1.plot(irr_arr_ab[:, prof_index, 0]/ (irr_arr_ab[-1, prof_index, 0]+irr_arr_ab[-1, prof_index, 1]), irr_arr_ab[:, prof_index, 3], '--', label=f'Model Using OOI ab {lam}', color=colors[k], linewidth=1.5)
        ...
        ax1.plot(spkir[:,lam_i], spkir_depth, '-', label=f'OOI SPKIR {lam}', color=colors[k], linewidth=1.5)
    ...
    axa.set_ylabel(f"Z [m]")
    #ax.set_xlabel(f"Downwelling Spectral Irradiance {spkir_dat.attrs['units']}")

    axa.set_xlabel(r"OOI Absorption [$m^{-1}$]")
    axb.set_xlabel(r"OOI Scattering [$m^{-1}$]")

    ax1.set_xlabel(r"$E_d+E_s$ "+ f"{spkir_prof['spkir_abj_cspp_downwelling_vector'].attrs['units']}")
#    ax1.set_xlabel( r"$\frac{E_d}{E_{d0} + E_{s0}}$ ", fontsize=14) 
    ...
    ax1.set_title(f"b) Normalized Downward Direct")
    ax2.set_title(f"c) Normalized Downward Diffuse")
    ax3.set_title(f"d) Normalized Upwelling")
...
def Plot_Correlation_Irradiance_Hoff(N, spkir_profs, optaa_profs, phy_species, wavelengths, irr_fields, irr_fields_ab): 
    ...
    Ed_relbias_gen = np.zeros((N, Nprof))
    perc_ab = np.zeros(Nprof)
    perc_gen = np.zeros(Nprof)
...
#        mean_c[k] = np.mean(c)
        ...
        Ed_relbias[:,k] = (y-x)/x0
        y = irr_fields[0][lam][:, k, 0] + irr_fields[0][lam][:, k,1]
        Ed_relbias_gen[:,k] = (y-x)/x0
        ...
        for j in range(N-1, 0, -1): 
            if irr_fields[0][lam][j, k, 0] + irr_fields[0][lam][j, k,1] < 0.01*(irr_fields[0][lam][-1, k, 0] + irr_fields[0][lam][-1, k,1]): 
                perc_gen[k] = irr_fields[0][lam][j,k,3] 
                break
            else: 
                perc_gen[k] = irr_fields[0][lam][j,k,3] 
...
#        mean_c[k] = irr_fields_ab[0][lam][-1,k,0] + irr_fields_ab[0][lam][-1,k,1]
    ...
    Ed_relbias_gen = np.reshape(Ed_relbias_gen[mask], (N, Nprof-badprof))
    perc_ab = perc_ab[mask[0,:]]
    perc_gen = perc_gen[mask[0,:]]
    perc_spkir = perc_spkir[mask[0,:]]

    
    print('mean 1% light level bias ab :', np.mean(perc_ab-perc_spkir))
    print('mean 1% light level bias gen :', np.mean(perc_gen-perc_spkir))
...
#    im = ax.pcolormesh(dt_posmesh, zmesh, Ed_relbias, shading = 'gouraud', cmap ='gist_rainbow')
    ...
    vmax = 0.5
    cmap = 'nipy_spectral'
    im1 = ax.pcolormesh(dt_posmesh, zmesh, Ed_relbias, cmap =cmap, vmin =vmin, vmax = vmax)
    im2 = axg.pcolormesh(dt_posmesh, zmesh, Ed_relbias_gen, cmap =cmap, vmin =vmin, vmax=vmax)
    ax.scatter(mean_c_pos, perc_ab, marker='o', linewidth =1.5, facecolor ='none', edgecolor ='black', label=r'1% Model Using OOI ab')
    axg.scatter(mean_c_pos, perc_gen, marker='o', linewidth =1.5, facecolor ='none', edgecolor ='black', label=r'1% Model Generic')
    ax.scatter(mean_c_pos, perc_spkir, marker='o', linewidth = 1.5, facecolor ='none', edgecolor ='white', label=r'1% OOI')
    axg.scatter(mean_c_pos, perc_spkir, marker='o', linewidth = 1.5, facecolor ='none', edgecolor ='white', label=r'1% OOI')
...
#    cbar.set_label(r'$\frac{E_{d_{\mathrm{model}}} - E_{d_{\mathrm{OOI}}}}{E_{d_{\mathrm{OOI}}}}$', rotation = 90, fontsize = 16)
    cbar.set_label(r'Relative Bias of Modeled $E_d+E_s$ to OOI $E_d+E_s$', rotation = 90, fontsize = 12)


    dt_pos = dt_pos[0:-1:3]
    dt_labs = [dt_labs[k] for k in dt_pos]
    axg.set_xticks(dt_pos)
    axg.set_xticklabels(dt_labs, rotation=65, fontsize=8)
    axg.set_ylabel('Z [m]')
    ax.set_ylabel('Z [m]')
    axg.legend()
    ax.legend()

    ax.set_title("a) OOI Absorption and Scattering")
    axg.set_title("b) Modeled Absorption and Scattering Using Generic Coefficents")
...
if __name__ == '__main__':
    ...
    irrsavefile_head = 'ooi_irr_out/irr_field_bwat'
    site = "CE02SHSP"
    node = 'SP001'
